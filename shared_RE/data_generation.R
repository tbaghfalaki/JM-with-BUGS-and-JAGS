...
death <- rep(1, nsample) # 1=observed, 0=censored
...
r1=c(0.5,-0.5)
mut=surt=rep(0,nsample)
r=1
for(i in 1:nsample){
  mut[i]<-(alpha[1]+alpha[2]*w1[i]+alpha[3]*w2[i]+r1[1]*u[i, 1]+r1[2]*u[i, 2])
}

for (i in 1:nsample) {
  time[i] = rweibull(1,r,exp(-mut[i]/r))
}
for (i in 1:nsample) {
  u=runif(1)
  if(time[i] >= 2)((time[i]=2) & (death[i]=0))
}
...
save(long.data, file = "/Users/taban/Desktop/Taban/joint modeling bugs/Simulated_data/long.data_sre_1.RData")
save(surv.data, file = "/Users/taban/Desktop/Taban/joint modeling bugs/Simulated_data/surv.data_sre_1.RData")
